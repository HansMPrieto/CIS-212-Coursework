...
/*
 * traverses arraylist, calling jsFree on each element
 */
static void purge(AlData *ald, void (*jsFree)(void *element)) {

    if (jsFree != NULL) {
        long i;

        for (i = 0L; i < ald->size; i++) {
            (*jsFree)(ald->theArray[i]); /* user frees element storage */
            ald->theArray[i] = NULL;
        }
    }
}
...
static void al_destroy(const ArrayList *al, void (*freeFxn)(void *element)) {
    ...
    purge(ald, freeFxn);
    jsFree(ald->theArray);		  /* we free array of pointers */
    jsFree(ald);
    jsFree((void *)al);			  /* we free the ArrayList struct */
}
 ...
 *
 * returns pointer to duplicate array or NULL if jsMalloc failure
 */
static void **arraydupl(AlData *ald) {
    void **tmp = NULL;
    if (ald->size > 0L) {
        size_t nbytes = ald->size * sizeof(void *);
        tmp = (void **)jsMalloc(nbytes);
        if (tmp != NULL) {
            long i;
...
const ArrayList *ArrayList_create(long capacity) {
    ArrayList *al = (ArrayList *)jsMalloc(sizeof(ArrayList));

    if (al != NULL) {
        AlData *ald = (AlData *)jsMalloc(sizeof(AlData));
        if (ald != NULL) {
            long cap;
            void **array = NULL;

            cap = (capacity <= 0) ? DEFAULT_CAPACITY : capacity;
            array = (void **) jsMalloc(cap * sizeof(void *));
            if (array != NULL) {
                ald->capacity = cap;
                ald->size = 0L;
                ald->theArray = array;
