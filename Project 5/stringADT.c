 ...
 *
 * returns pointer to the copy, or NULL if jsMalloc() failure
*/
static char *slice_copy(char *buf, int begin, int end) {
    int size = end - begin + 1 + 1; /* +1 for '\0' */
    char *p;

    if ((p = (char *)jsMalloc(size)) != NULL) {
        int i, j;
...
static const String *st_copy(const String *str) {
    ...
    String *nstr = (String *)jsMalloc(sizeof(String));

    if (nstr != NULL) {
        StData *nstd = (StData *)jsMalloc(sizeof(StData));
...
static const String *st_slice(const String *str, int begin, int end) {
    ...
    if (begin >= 0 && end <= std->length && begin < end) {
        nstr = (String *)jsMalloc(sizeof(String));
        if (nstr != NULL) {
            StData *nstd = (StData *)jsMalloc(sizeof(StData));
...
static int st_append(const String *str, char *suffix) {
    ...
    jsStrcpy(std->buf+std->length, suffix);
    std->length = jsStrlen(std->buf);
    return 1;
}
...
static int st_replace(const String *str, char *old, char *new) {
    StData *std = (StData *)str->self;
    int oldlen = jsStrlen(old);
    int newlen = jsStrlen(new);
    int nold, size;
    char *nbuf;
    int result = 0; /* assume jsMalloc() fails */

    nold = ((std->length) / oldlen) + 1; /* worst case # of matches */
    size = std->size - nold * oldlen + nold * newlen;
    if (size < std->size)
        size = std->size;
    nbuf = (char *)jsMalloc(size);
    if (nbuf != NULL) {
        int i, j;
        i = 0;
        j = 0;
        while (i < std->length) {
            if (jsStrncmp(std->buf+i, old, oldlen) == 0) {
...
static int st_compare(const String *str, const String *other) {
    ...
    return jsStrcmp(std->buf, otd->buf);
}
...
static int st_endsWith(const String *str, char *suffix, int begin, int end) {
    ...
    nchars = end - begin;
    suflen = jsStrlen(suffix);
    if (nchars >= suflen) {
        if (jsStrncmp(suffix, std->buf + end - suflen, suflen) == 0)
            result = 1;
    }
    return result;
}
...
static int st_index(const String *str, char *substr, int begin, int end) {
    ...
    for (i = begin, j = end - begin; i < end && j >= n; i++, j--)
        if (jsStrncmp(std->buf + i, substr, n) == 0)
            return i;
    return -1;
}
...
static int getword(char buf[], int i, char word[]) {
    ...
    p = word;
    while (buf[j] != '\0') {
        if (jsStrchr(whitespace, buf[j]) != NULL)
            break;
        *p++ = buf[j];
        j++;
    }
...
static int st_startsWith(const String *str, char *prefix, int begin, int end) {
    ...
    nchars = end - begin;
    prelen = jsStrlen(prefix);
    if (nchars >= prelen) {
        if (jsStrncmp(prefix, std->buf + begin, prelen) == 0)
            result = 1;
    }
    return result;
}
...
const String *String_create(char *str) {
    String *st = (String *)jsMalloc(sizeof(String));

    if (st != NULL) {
        StData *std = (StData *)jsMalloc(sizeof(StData));

        if (std != NULL) {
            std->length = jsStrlen(str); /* length of str */
            std->size = std->length + 1; /* account for '\0' */
            std->buf = (char *)jsMalloc(std->size);
            if (std->buf != NULL) {
                jsStrcpy(std->buf, str);
                *st = template;
                st->self = std;
            } else {
                jsFree(std);
